...
import ProximaButton from '@/button/button.vue';
import IconCross from '@/icon/cross.vue';

// Expose

const el = ref({} as InstanceType<typeof ProximaButton>);
...
export const componentName = 'ProximaButtonClose';

/**
 * [Live Demo](https://proxima.wiki/button/close) | [Code on Github](https://github.com/werty1001/proxima/blob/main/src/button/close.vue)
 *
 * @example
 * ```html
 * <ProximaButtonClose size="s" />
 * ```
 *
 * @prop text `string` — Button text (default `''`)
 * @prop href `string` — Will turn the button into a link (default `''`)
 * @prop icon `object` — Icon component (default `IconCross`)
 ...
 * @prop shadow `string` — Button shadow: `soft` / `wide` (default `''`)
 * @prop effect `string` — Click effect: `none` / `ripple` / `pulse` (default `none`)
 * @prop theme `string` — Button theme (default `'dim'`)
 *
 * @since v0.1.0
 */
